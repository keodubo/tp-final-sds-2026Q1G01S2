...
ruta valida el no-solapamiento en cada inserción y en cada paso. La CLI registra el estado
...
($N=30$, abajo), contacto puro, $p=0{,}1$. Color según velocidad. Animaciones
...
el sistema se agrupa a contacto detrás del más lento a cualquier densidad, de modo que la
velocidad queda fijada por el mínimo de los $N$ sorteos de $v_{\max}$. La caída suave de
...
$\approx44{,}5\unit{mm/s}$, pero a $p\ge0{,}3$ el sistema se congela casi por completo
($\bar v\approx0{,}45\unit{mm/s}$ a $p=0{,}3$ y $0{,}16\unit{mm/s}$ a $p=0{,}4$), por el
``trinquete'' de que los $30$ vehículos deben no frenar en el mismo paso para avanzar
($(1-p)^{30}\approx0$). Ese congelamiento no corresponde a ningún régimen del experimento
...
decreciente (medio) y aleatorio (abajo) (contacto puro, $p=0{,}1$). Animaciones
disponibles en el repositorio del grupo.}
\label{fig:anim-inc}
\end{figure}
...
\begin{itemize}
  ...
  ($\Delta v=6\unit{mm/s}$) con pico en $\approx90\unit{mm/s}$, mientras el experimento
  tiene una distribución suave con pico en $\approx65\unit{mm/s}$ y cola hasta
  $\approx300\unit{mm/s}$. Al agruparse detrás del más lento, el modelo no puebla
  velocidades por debajo de la del agente más lento presente.
