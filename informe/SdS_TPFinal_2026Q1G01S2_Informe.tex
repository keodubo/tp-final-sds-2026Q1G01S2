...
\title{\textbf{Diagrama fundamental de \emph{vibration-driven vehicles}:
reproducción de tendencias con un autómata celular de Nagel--Schreckenberg
con interacción por contacto}}
\author{Grupo G01S2 \\
        \textbf{[COMPLETAR: nombres y legajos de los integrantes]} \\
...
\emph{vibration-driven vehicles} (VDV) medidas experimentalmente por Patterson y
...
por contacto}, sin sensado remoto: robots comerciales \emph{vibration-driven} (Hexbug
Nano, cuerpo de $44\times15\times18\unit{mm}$) confinados en un canal circular de
...
R3 nunca se aplica y el esquema coincide con el Nagel--Schreckenberg canónico (no se
consume el generador de números aleatorios).
...
cíclicamente sobre la ruta y ejecuta cada paso sincrónico sobre un \emph{snapshot}
inmutable de huecos y velocidades deseadas, de modo que el resultado no depende del
orden de iteración. La regla de colisión recibe ese snapshot y devuelve, por vehículo,
...
paso():
    ...
    movimientos <- ReglaColisión.resolver(snapshot(huecos, deseadas))     # R2
...
toda la aleatoriedad proviene de un único generador sembrado con ese identificador,
...
ruta valida el no-solapamiento en cada inserción y en cada paso.
...
$\rho=N/L_{\mathrm{fís}}$, que a $N=30$ vale $0{,}0227\unit{mm^{-1}}=1/44\unit{mm}$.
...
determinada por su identificador reproducible (que en la línea de comandos se pasa como
\texttt{-{}-seed}).
...
\begin{itemize}
  ...
  (\emph{ascending}, los más lentos primero), decreciente (\emph{descending}, los más
  rápidos primero) y aleatorio (\emph{random}).
\end{itemize}
...
media móvil). El \textbf{corte del estacionario} se elige \emph{por inspección} de la
evolución temporal (no por un descarte fijo en porcentaje) y queda registrado, por punto,
en \texttt{figures/manifiesto.csv}. En el protocolo incremental, por su diseño, el
...
\includegraphics[width=0.48\linewidth]{N5_p01_CONTACTO_PURO_FIXED_N_oe1_r1_fotograma.png}\hfill
\includegraphics[width=0.48\linewidth]{N30_p01_CONTACTO_PURO_FIXED_N_oe1_r1_fotograma.png}
\caption{Fotogramas en régimen a baja densidad ($N=5$, izquierda) y a saturación
($N=30$, derecha), contacto puro, $p=0{,}1$. Color según velocidad. Animaciones:
\href{https://youtu.be/XXXX}{[N=5]} \quad \href{https://youtu.be/XXXX}{[N=30]}.}
...
cercana a la velocidad libre media y prácticamente no depende de $N$; al acercarse a la
ruta llena cae. En el punto singular $N=30$ (ruta exactamente llena) el valor de
saturación depende de $p$: con $p=0$ el anillo rígido cruza a la velocidad del más lento,
y con $p>0$ el frenado aleatorio del cúmulo lo hace caer por debajo de ella.
...
\begin{figure}[h]\centering
\includegraphics[width=0.32\linewidth]{INC_p01_CONTACTO_PURO_ASCENDING_oe10_r1_fotograma.png}\hfill
\includegraphics[width=0.32\linewidth]{INC_p01_CONTACTO_PURO_DESCENDING_oe10_r1_fotograma.png}\hfill
\includegraphics[width=0.32\linewidth]{INC_p01_CONTACTO_PURO_RANDOM_oe10_r1_fotograma.png}
\caption{Fotogramas del protocolo incremental para los órdenes creciente, decreciente y
aleatorio (contacto puro, $p=0{,}1$). Animaciones:
\href{https://youtu.be/XXXX}{[creciente]} \href{https://youtu.be/XXXX}{[decreciente]}
\href{https://youtu.be/XXXX}{[aleatorio]}.}
\label{fig:anim-inc}
\end{figure}
...
y la PDF de velocidad se angosta al aumentar $N$, de forma más marcada en el orden
creciente.
...
\begin{itemize}
  ...
  velocidad con $N$ reproducen las distribuciones del experimento, con la salvedad de que
  el modelo no genera la cola de densidad por solapamiento (Sec.~\ref{sec:limitaciones}).
