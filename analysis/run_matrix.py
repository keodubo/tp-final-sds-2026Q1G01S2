...
guardando cada salida en ``data/``. Una **realización** = una semilla.
...
def build_command(jar: str, out_dir: Path, n: int, p: float, rule: str,
                  seed: int, steps: int, output_every: int) -> tuple[list[str], Path]:
    tag = f"N{n}_p{p}_{rule}_r{seed}".replace(".", "")
...
def main() -> None:
    ...
    ap.add_argument("--realizations", type=int, default=30, help="cantidad de realizaciones (semillas)")
    ...
    combos = list(itertools.product(args.n, args.p, args.rule, range(1, args.realizations + 1)))
    print(f"{len(combos)} corridas (N={args.n} × p={args.p} × reglas={args.rule} × {args.realizations} realizaciones)")

    for n, p, rule, seed in combos:
        cmd, out_file = build_command(args.jar, out_dir, n, p, rule, seed, args.steps, args.output_every)
        if args.dry_run:
            print(" ".join(cmd))
        else:
