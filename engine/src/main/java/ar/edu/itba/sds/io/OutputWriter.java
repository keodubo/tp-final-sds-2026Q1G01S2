...
public final class OutputWriter implements AutoCloseable {
    ...
    private void writeHeader(Config c) {
        ...
        line(String.format(Locale.US,
                "# N=%d p=%.6f vfree_min_mmps=%.3f vfree_max_mmps=%.3f seed=%d steps=%d output_every=%d",
                c.n(), c.brakeProb(), c.freeSpeedMinMmS(), c.freeSpeedMaxMmS(), c.seed(), c.steps(), c.outputEvery()));
