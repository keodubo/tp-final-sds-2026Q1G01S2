...
%  (20 diapositivas numeradas + 3 separadores de sección).
...
\begin{center}
  ...
  \hspace{1.5em}|\hspace{1.5em}\textbf{Objetivo: $\sim$13 min (dentro de 10--15)}\\[2pt]
  {\small Sincronizado con las 20 diapositivas numeradas (+3 separadores de sección) de \texttt{Presentacion.pdf}. Online: cada uno habla su bloque de corrido.}
\end{center}
...
Motor, corridas y dinámica (incremental) & 7--13 & \textcolor{cian}{\textbf{Ian}} & $\sim$4:30 \\
Resultados del orden, sensibilidad y cierre & 14--20 & \textcolor{ckeo}{\textbf{Keo}} & $\sim$4:30 \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Videos:} las diapos \textbf{9} y \textbf{12} muestran un \textbf{fotograma fijo} (en el PDF no van
...
Gracias. El motor está hecho en Java, y lo pensamos así: cada paso se calcula sobre una \textbf{instantánea
inmutable} de los huecos y las velocidades, así que el resultado \textbf{no depende del orden} en que recorrés los
vehículos. El no-solapamiento sale \textbf{garantizado por construcción}, con esa desigualdad del desplazamiento,
y la inserción incremental la hacemos \textbf{por empuje}, que nos deja llegar a los 30 exactos. En el
pseudocódigo se ven las cuatro reglas de cada paso, R1 a R4. Y es \textbf{determinista} por realización. Como
dato, el motor saca \textbf{solo} posición y velocidad; el color y los observables los calculamos después.

\diapo{8}{Simulaciones y observables}{IAN}{cian}{45 s}
Del lado de las corridas: barremos el número de vehículos de 5 a 30, y la probabilidad de frenado de 0 a 0,4.
Dos protocolos, uno de $N$ fijo y el incremental que va de 5 a 30, con \textbf{30 realizaciones} por punto. Y
todos los observables se calculan \textbf{después} de la simulación. Ahí están las fórmulas: la velocidad media
es el promedio sobre vehículos y sobre la \textbf{ventana estacionaria} $T$, y el error es el \textbf{desvío
entre realizaciones}, no el de todos los cuadros. Sacamos también la densidad de cada uno, las distribuciones y
el diagrama fundamental. El estacionario lo elegimos \textbf{a ojo, por inspección}.

\diapo{9}{Dinámica $N$ fijo (fotograma)}{IAN}{cian}{55 s}
...
\diapo{11}{Velocidad media vs $N$ ($N$ fijo)}{IAN}{cian}{45 s}
...
\diapo{12}{Efecto del orden (fotograma)}{IAN}{cian}{50 s}
...
\textbf{orden de inserción}.

\diapo{13}{Evolución temporal incremental}{IAN}{cian}{35 s}
Y así se ve en el tiempo: la \textbf{escalera} del incremental. Cada 180 segundos entran 5 vehículos y la
velocidad media pega un salto y se reacomoda. Cada escalón es un $N$ distinto, y el valor de cada fase lo
promediamos sobre su \textbf{ventana completa de 180 segundos}, igual que el paper. \relevo{Ahí te paso, Keo, con
la curva por orden.}

% ============================ BLOQUE 3 --- KEO (14-20) ======================
\diapo{14}{Velocidad media vs $N$ por orden}{KEO}{ckeo}{45 s}
Gracias, Ian. Esta es la curva análoga a la \textbf{Figura 2} del paper: la velocidad media según el orden. El
...
\diapo{15}{Distribuciones por orden}{KEO}{ckeo}{40 s}
...
\diapo{16}{Diagrama fundamental por orden}{KEO}{ckeo}{35 s}
...
\diapo{17}{Validación del motor}{KEO}{ckeo}{40 s}
...
\diapo{18}{Sensibilidad ($\Delta x$, $\Delta t$, $L$)}{KEO}{ckeo}{45 s}
...
\diapo{19}{Limitaciones}{KEO}{ckeo}{45 s}
...
\diapo{20}{Conclusiones}{KEO}{ckeo}{40 s}
...
\item \textbf{¿Estudiaron la sensibilidad a $dt$, $L$ y $\Delta x$?} Sí (diapo 18). $L$ irrelevante; $\Delta x$
