...
\begin{frame}{Lo que midió el experimento}
\begin{itemize}
...
% ---- El paso del motor (pseudocódigo de la simulación principal) ------------
\begin{frame}{El paso del motor}
\centering
{\small
\begin{block}{Bucle principal de la simulación (contacto puro)}
\begin{tabbing}
\quad\=\quad\=\kill
inicializar la ruta con los vehículos (velocidad $0$)\\[2pt]
\textbf{por cada paso} $t=0,\dots,T-1$:\\
\>registrar el estado $(x_i,v_i)$ \hfill\textit{// salida física}\\
\>[incremental] si toca lote: insertar $5$ vehículos (por empuje)\\
\>\textbf{para cada vehículo} $i$ (sobre la instantánea inmutable):\\
\>\>R1: $\hat v_i\leftarrow\min(v_i+1,\,v_{\max,i})$\\
\>\>R3: con prob.\ $p$: $\hat v_i\leftarrow\max(0,\,\hat v_i-1)$\\
\>R2: $d_i\leftarrow\min(\hat v_i,\;g_i+d_{i+1})$ \hfill\textit{// contactos, sin solapar}\\
...
\centering
\includegraphics[height=0.8\textheight]{evolucion_temporal_contacto_puro_sin_orden_fixed.png}
\end{frame}
...
% ---- ANIMACIÓN (estudio orden) — creciente ---------------------------------
\begin{frame}{Efecto del orden: inserción creciente}
\centering
\videohero{INC_p01_CONTACTO_PURO_ASCENDING_r1}{\linewidth}\\[4pt]
{\footnotesize Incremental $5\to30$, $p=0{,}1$ --- los más lentos primero. Color $=$ velocidad.}\\ \playnota
\end{frame}

% ---- ANIMACIÓN (estudio orden) — decreciente -------------------------------
\begin{frame}{Efecto del orden: inserción decreciente}
\centering
\videohero{INC_p01_CONTACTO_PURO_DESCENDING_r1}{\linewidth}\\[4pt]
{\footnotesize Incremental $5\to30$, $p=0{,}1$ --- los más rápidos primero. Color $=$ velocidad.}\\ \playnota
\end{frame}

% ---- ANIMACIÓN (estudio orden) — aleatorio ---------------------------------
\begin{frame}{Efecto del orden: inserción aleatoria}
\centering
\videohero{INC_p01_CONTACTO_PURO_RANDOM_r1}{\linewidth}\\[4pt]
{\footnotesize Incremental $5\to30$, $p=0{,}1$ --- orden aleatorio. Color $=$ velocidad.}\\ \playnota
\end{frame}
...
fase se promedia sobre su ventana completa de $180\unit{s}$.}
\end{frame}

% ---- curva respuesta-estímulo (orden) --------------------------------------
\begin{frame}{Velocidad media vs.\ $N$ por orden}
\centering
...
\begin{frame}{Distribución de densidad por orden}
\centering
\includegraphics[height=0.78\textheight]{pdf_densidad_incremental_contacto_puro_ascending_p01_incremental.png}\\
{\footnotesize Orden creciente: la \textbf{densidad} pica en $1/(44\unit{mm})$ y el pico se acentúa al subir $N$.}
\end{frame}

% ---- distribuciones: velocidad ---------------------------------------------
\begin{frame}{Distribución de velocidad por orden}
\centering
\includegraphics[height=0.78\textheight]{pdf_velocidad_incremental_contacto_puro_ascending_p01_incremental.png}\\
{\footnotesize Orden creciente: la \textbf{velocidad} queda casi independiente de $N$ (el más lento está desde $N=5$).}
\end{frame}
